...
bool HGE_CALL HGE_Impl::System_Initiate()
{
    ...
    style_windowed_ = WS_BORDER | WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_VISIBLE;
...
void HGE_CALL HGE_Impl::System_SetStateBool( const hgeBoolState state, const bool value )
{
    ...
    case HGE_WINDOWED :
        ...
        {
            if ( d3dpp_windowed_.BackBufferFormat == D3DFMT_UNKNOWN || d3dpp_fullscreen_.BackBufferFormat == D3DFMT_UNKNOWN )
            {
                break;
            }
...
hgeGAPIDevice *HGE_CALL HGE_Impl::GetGAPIDevice()
{
    return d3d_device_;
}
...
LRESULT HGE_Impl::window_proc( HWND hwnd, const UINT msg, WPARAM wparam, LPARAM lparam )
{
    ...
    case WM_PAINT :
        if ( hgeImpl::pHGE->d3d_ && hgeImpl::pHGE->proc_render_func_ && hgeImpl::pHGE->windowed_ )
        {
    ...
    case WM_ACTIVATE :
    {
        ...
        if ( hgeImpl::pHGE->d3d_ && hgeImpl::pHGE->active_ != activating )
        {
    ...
    case WM_SETCURSOR :
        if ( hgeImpl::pHGE->active_ && LOWORD( lparam ) == HTCLIENT && hgeImpl::pHGE->hide_mouse_ )
        {
            SetCursor( nullptr );
        }
        else
    ...
    case WM_SYSKEYDOWN :
    {
        if ( wparam == VK_F4 )
        {
            if ( hgeImpl::pHGE->proc_exit_func_ && !hgeImpl::pHGE->proc_exit_func_() )
            {
                return FALSE;
            }
    ...
    case WM_SIZE :
        if ( hgeImpl::pHGE->d3d_ && wparam == SIZE_RESTORED )
        {
